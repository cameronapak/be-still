...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  try {
    ...
    const isAppropriate = response.choices[0].message.content?.trim().toLowerCase().includes("yes");

    return new Response(
      JSON.stringify({
        isAppropriate,
        message: isAppropriate ? "Name is appropriate" : "Please use a more appropriate name"
      }),
      {
        status: 200,
        headers: {
